...
Let $A$ be the data matrix, then if it is constructed by the Gaussian kernel
\begin{equation}
	\label{eq:gaussian}
...
\begin{equation}
	N = 300,~~ n_K = 100, ~~ n_f = 28, ~~ n_s = 5, ~~ n_L = n_fn_s = 140,
\end{equation}
which are then extended in a \textit{binary search} manner. Each \codeword{data_setup()} process roughly takes around 300s. The fitting is done using \codeword{CGLS()} with $\codeword{itmax} = 500$ and $\codeword{t_limit} = 900$s, by using only the functions to compute $Au$ and $A^\top v$ as the input variable. Currently active learning scheme has not been tested.
...
The true error of the fitting is described by the mean absolute error (MAE)
\begin{equation}
	\text{MAE} := \frac{1}{N_{QM9}}\sum_{m \in W}|E_{m} - V_K(w_m)|.
\end{equation}
Several hand-picked fitting experiment results with the lowest MAE are shown in table \ref{tab:exp}.


\begin{table}[H]
	\centering
	\caption{Selected fitting experiment results. Here $N_\text{QM9} = 133628$, $n_K = 100$, and $n_U = 200$ for all of the experiments. \textbf{ft\_sos} indicates the inclusion of sums of squares features meanwhile \textbf{ft\_bin} indicates the inclusion of binomial features; \textbf{t\_solver} indicates the time spent by CGLS; and \textbf{t\_pred} indicates the energy prediction time}
	\begin{tabular}{|l|l|l|l|l|l|l|l|}
		\hline
		\multicolumn{1}{|c|}{\textbf{$n_{af}$}} & \multicolumn{1}{c|}{\textbf{$n_{mf}$}} & \multicolumn{1}{c|}{\textbf{$n_s$}} & \multicolumn{1}{c|}{\textbf{MAE (kcal/mol)}} & \multicolumn{1}{c|}{\textbf{ft\_sos}} & \multicolumn{1}{c|}{\textbf{ft\_bin}} & \multicolumn{1}{c|}{\textbf{t\_solver}} & \multicolumn{1}{c|}{\textbf{t\_pred}} \\ \hline
		4                                    & 18                                  & 5                                      & 2335.67069                        & 0                                     & 0                                     & 409.0                                   & 93.0                                  \\ \hline
		4                                    & 19                                  & 5                                      & 2413.86345                        & 0                                     & 0                                     & 445.0                                   & 113.0                                 \\ \hline
		4                                    & 26                                  & 5                                      & 2623.45732                        & 0                                     & 0                                     & 566.0                                   & 160.0                                 \\ \hline
		4                                    & 23                                  & 5                                      & 2647.25459                        & 1                                     & 0                                     & 529.0                                   & 136.0                                 \\ \hline
		6                                    & 28                                  & 5                                      & 2651.13104                        & 0                                     & 0                                     & 623.0                                   & 179.0                                 \\ \hline
	\end{tabular}
	\label{tab:exp}
\end{table}
...
\begin{equation}
	(8\times 10^{-6}) \times 133628 \times 300 = 320.7072 \text{ MB},
\end{equation}
...
\begin{equation}
	(8\times 10^{-6}) \times 133628 \times 100 \times 140 = 14966.336 \text{ MB} = 14.9 \text{ GB}
\end{equation}
memory size. Hence mini-batching is necessary, with \codeword{batchsize = 10_000}, for each mini-batch loop the size of $\phi$ becomes $1/13$ of its actual size which is around 1GB, this fits to the 7.7 GB memory limitation. 
