...
Let us see an example of the SMILES string of \codeword{dsgdb9nsd_066222.xyz} geometry from the QM9 dataset:
\begin{equation*}
...
The rest of the experiments are conducted to find out the errors of mostly the kernel models with $E - E^{\text{null}} - E^{\text{sob}}$ target. One thing to note is that even without dataset pre-selection, a lower MAE than the previous best which was 10.914 \at{add details of prev results later in tuning sections} is found, which is 9.129; experiments with data selections will be conducted in later series. 
