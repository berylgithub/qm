...
\begin{equation}
    ...
    \begin{split}
        \Delta_{j,K}(w)&= \frac{\D \left(\sum_{k\in K} \frac{E_k + \sum_l \psi_{kl}}{D_k} \right)/ S_K - \big(E_j + \sum_l \psi_{jl}\big)}{\alpha_j} \\
...
\begin{equation}
	...
	\end{split}
\end{equation}
or more explicitly, with the intermediate values in (\refeq{eq:intermediate}) already pre-computed
\begin{equation}
...
Currently, all experiments are run on several hand-picked compounds which contains $n_\text{data} \geq 250$, which are
