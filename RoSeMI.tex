...
\begin{itemize}
	...
	\item normalization mode: whether to normalize atomic features \codeword{norm_af} $ \in \{\codeword{true, false}\}$ or molecular features \codeword{norm_mf} $ \in \{\codeword{true, false}\}$.
...
\begin{itemize}
	...
	\item index $k \in \mathbb{Z}_{\geq 0}$ of precomputed set of selected centers by the \codeword{usequence}.
\end{itemize}

For model fitting, the model choice is \codeword{model} $\in$ \{RoSeMI, KRR, NN, LLS, GAK\}. The hyperparameter for both KRR and GAK is the scaling factor $c\sigma \in \mathbb{Z}_{>0}$. For NN, there are several hyperparameters:
\begin{itemize}
...
\begin{table}[H]
	\centering
	\caption{The hyperparameters passed into the hyperparameter optimization routine with its data type, possible range, best found value, and starting value. \textbf{Int} type means integer, \textbf{Cat} type means categorical.}
	\begin{tabular}{|c|c|c|c|c|}
		\hline
		\textbf{hyperparam}	& \textbf{type}	& \textbf{range}	& \textbf{best}	& \textbf{start}	\\ \hline
		$p_{af}$	& \codeword{Int}	& [1,[51,165,140]]	& 45	& 82	\\ \hline
		$p_{mf}$	& \codeword{Int}	& $[1,p_{af}]$	& 28	& 41	\\ \hline
		ns	& \codeword{Int}	& [1,10]	& 7	& 6	\\ \hline
		ftype	& \codeword{Cat}	& [1,3]	& 1	& 2	\\ \hline
		norm\_af	& \codeword{Cat}	& [0,1]	& 0	& 0	\\ \hline
		norm\_mf	& \codeword{Cat}	& [0,1]	& 0	& 0	\\ \hline
		$k$	& \codeword{Int}	& [0,95]	& 48	& 48	\\ \hline
		model	& \codeword{Cat}	& [1,5]	& 4	& 4	\\ \hline
		$c\sigma$	& \codeword{Int}	& $[2^0,2^{20}]$	& 524287	& 524288	\\ \hline
	\end{tabular}
