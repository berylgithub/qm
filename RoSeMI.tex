...
\begin{itemize}
	\item Compute the atomic features, this gives a row vector $f^l_{i:}$ with length $n_{af}$ for each atom $i$ in a molecule $l$. Apply this to all molecules in the dataset, hence there are a total of $N_\text{QM9}$ matrices $f^l$.
...
\begin{equation}
	\label{eq:gaussian}
	A_{ij} = \text{exp}\Big(-\frac{\|w_{i:} - w_{j:}\|^2}{2\sigma}\Big),
\end{equation}
...
\begin{equation}
	\sigma_0 = \frac{1}{Nn_K}\sum_{ij}\text{exp}(-\|w_{i:} - w_{j:}\|^2).
\end{equation} 
...
\subsection{FCHL}
The entries of FCHL's data matrix are \at{\cite[Eq. (10)]{Faber et al} from \url{https://aip.scitation.org/doi/10.1063/1.5126701}}
\begin{equation}
	A_{l' l} = \sum_P \sum_{Z^{l'}_i = P} \sum_{Z^{l}_j = P} A^\text{atom}(f^{l'}_i, f^l_j)
\end{equation}
where $P$ is the set of index of atom types, $Z_i^l$ is the nuclear charge of atom $i$ in molecule $l$, and the atomic Gaussian kernel function is \at{\cite[(26)]{Faber et al}}
\begin{equation}
