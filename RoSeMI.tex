...
\begin{equation}
	...
	\end{split}
\end{equation}
The set of index $K$ depends entirely from the set of features used, it affects the resulting atomic energies. The result of the atomic energies fitting with the lowest MAE with the molecular energies $E$ as the target is shown in table \ref{tab:atomic_exp}. 
...
The fitting experiments with $E^\text{red}$ as the target are run by varying the models and the set of features, in total there are 70 experiment instances, 5 instances with the lowest MAE are shown in table \ref{tab:exp_reduced_energy_1}.
The experiments with the lowest mean MAE using the same set of features to see the comparison of the models are shown in table \ref{tab:exp_reduced_energy_2}. Here the MAEs are computed from the original energies, since the predicted energies are actually the predicted reduced energies,
\begin{equation}
...
The training set indices $K$ is computed from the processed ACSF set of features with $n_{af} = 4$ and $n_{mf} = 18$, hence it is not the most accurate molecular centers relative to FCHL set of features.
