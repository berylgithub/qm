...
\subsection{FCHL}
Faber--Christensen--Huang--Lilienfeld (FCHL) \at{cite Faber et al}. Essentially FCHL encodes radial distribution by the two-body term, and encodes the mean distances and angular information by three-body term. FCHL considers atomic coordinates and the nuclear charges as the primary inputs. For each molecule, FCHL returns an array of size $(\max_l n^l_\text{atom}, 5, n_\text{neigh})$, where $n_\text{neigh}$ is the number of atomic neighbours; almost in all cases $n_\text{neigh} = \max_l n^l_\text{atom}$, the only exception is when periodic boundary cell mode is used (usually for crystals). 
The hyperparameters are: cut-off radius (in Angstrom), number of neighbours, and periodic cells boundaries. For the QM9 dataset, FCHL returns an array with size $(133885, 29, 5, 29)$.
...
\section{Splines}
The splines (\texttt{bspline}) are used to increase the dimension of the feature space; with sufficient number of splines for each feature, it will lead to linearly separated data points.
\begin{itemize}
...
\begin{equation}
	\label{eq:gaussian}
	A_{ij} = \text{exp}\Big(-\frac{\|w_{i:} - w_{j:}\|^2}{2\sigma}\Big), ~~\text{for }i \in K, j \in K,
\end{equation}
where
\begin{equation}
	\sigma = c\sigma_0,
...
\subsection{FCHL}
The entries of FCHL's data matrix are
\begin{equation}
	A_{\hat{l}l} = \sum_{ij} A^\text{atom}(f^{\hat{l}}_i, f^l_j),
\end{equation}
where the atomic Gaussian kernel $A^\text{atom}$ is formed by
\begin{equation}
	A^\text{atom}(f^{\hat{l}}_i, f^l_j) = \delta_{Z_iZ_j} \text{ exp}\Big( - \frac{\|f^{\hat{l}}_i - f^l_j \|^2_2}{2 \sigma^2} \Big)
\end{equation}
where $\delta$ is the Kronecker delta which depends on the nuclear charge $Z_i$ of atom $i$ and nuclear charge $Z_j$ of atom $j$. 
For each pair of molecules, $A^\text{atom}$ is a sparse matrix with $\mathcal{O}(n_\text{atom}^2 - c)$ storage, where $c$ is the total number of occurrences of $\delta_{Z_iZ_j} = 0$. 
...
The main program is divided into two main subprograms: feature exctraction, data setup and fitter. Feature extraction refers to any operations included in extracting the raw array of features (usually on the atomic level) given the geometrical information of the molecules (e.g., coordinates, nuclear charges, etc), this is usually only done once for each set of molecular geometries. Data setup includes section (\ref{sec:feature}) up to section (\ref{sec:data}) except for the raw feature extraction; in general, everything that are independent of $\theta$ and the indexing from set $T$ are included into the data setup. Meanwhile the fitter subprogram computes everything which depend on $\theta$ or set $T$. The feature extraction and data setup subprogram is compatible with any types of model, while the fitter subprogram is specific for the Shepard model.
...
\begin{itemize}
	...
	\item Number of selected atomic features $n_{af} \in [1, n_{f}]\in \mathbb{Z}_{>0}$,
	\item molecular feature extraction mode, whether to include: sum of squares, binomial features;
	\item number of selected molecular features $n_{mf} \in [1, n_{f}]\in \mathbb{Z}_{>0}$,
	\item number of splines $n_{s}\in \mathbb{Z}_{>0}$.
\end{itemize}
For data selection, the only hyperparameter is $N \in [1, N_\text{QM9}] \in \mathbb{Z}_{>0}$, which is the number of centers requested. 
...
\end{itemize}
the NN can be optimized by several choices of optimization method: Adam, gradient descent, RMSProp, etc.
Meanwhile RoSeMI, KRR, and LLS are solved by CGLS hence we have a choice of the maximum iteration \codeword{itmax} $\in \mathbb{Z}_{>0}$.
