...
\begin{itemize}
	...
	\begin{equation}
		\sigma_j = \frac{1}{\sum_l n^l_\text{atom}}\sum_{li} (d_k)^l_{ij},
	\end{equation}
	where
	\begin{equation}
		(d_k)^l_{ij} := \max |f^l_{ij} - (\hat{f}_k)^l_{ij}|~~\text{ for }k = 1,2,...,
	\end{equation}
...
The Gaussian kernel and linear least squares can be fitted in a straightforward manner, the linear system is
\begin{equation}
	A \theta = b,
\end{equation}
...
\end{equation*}
we construct the $A$ explicitly here since it is cheap and fast to compute with only a maximum of 100 row size.
...
\begin{equation}
	N = 300,~~ n_K = 100, ~~ n_f = 28, ~~ n_s = 5, ~~ n_L = n_fn_s = 140, ~~ \codeword{t_limit} = 900 \text{seconds}
\end{equation}
...
\begin{table}[H]
	\centering
	\caption{Selected fitting experiment results. Here $N_\text{QM9} = 133628$, $n_K = 100$, and $n_U = 200$ for all of the experiments. \textbf{ft\_sos} indicates the inclusion of sums of squares features meanwhile \textbf{ft\_bin} indicates the inclusion of binomial features; \textbf{t\_solver} indicates the time spent by CGLS; and \textbf{t\_pred} indicates the energy prediction time. MAE is the mean absolute error of the energies in kcal/mol.}
