...
The data matrix of FCHL by default has the form of
\begin{equation}
	A_{ij} = \sum_{ij} A^\text{atom}(f^{\hat{l}}_i, f^l_j),
\end{equation}
...
For each pair of molecules, $A^\text{atom}$ is a sparse matrix with $\mathcal{O}(n_\text{atom}^2 - c)$ storage, where $c$ is the total number of occurence of $\delta_{Z_iZ_j} = 0$. 
