...
\begin{itemize}
    \item Compute the molecular features using ACSF (\at{cite Behler}). This would give a vector with length $= 51$ for each atom. Apply this to all molecules in the dataset hence there should be a total of $n_\text{mol}$ ACSF extractions.
    \item For each molecule containing $n_\text{atom}$ number of atoms, symmetrize the features by
        ...
        \begin{equation}
            F := (F_{S_1}, F_{S_2}, ..., F_{S_{51}}, F_{Q_1},F_{Q_2} ..., F_{Q_{51}}).
        \end{equation}
    	then we stack $F$ of each molecule as a row in the $\hat{F} \in \mathbb{R}^{n_\text{mol} \times n_\text{feature}}$ matrix.
