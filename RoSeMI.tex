...
\begin{equation*}
    ...
    \end{split}
\end{equation*}
By continuing from the last line of (\refeq{eq:delta}) in a simplified expression, then (\refeq{eq:delta}) can be expanded into
\begin{equation}
    \begin{split}
        ...
        &= \sum_k \frac{E_k}{D_kS_K\alpha_j} + \underbrace{\sum_{k,l} \frac{\theta_{k,l} \phi_{k,l} (1-D_kS_K)}{D_kS_K\alpha_j}}_{\text{if }k=j} +  \underbrace{\sum_{k,l} \frac{\theta_{k,l} \phi_{k,l}}{D_kS_K\alpha_j}}_{\text{if }k \neq j} - \frac{E_j}{\alpha_j}.\\
    \end{split}
\end{equation}
\iffalse
