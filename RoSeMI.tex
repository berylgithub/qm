...
\begin{itemize}
    \item For each molecule containing $n_\text{atom}$ number of atoms, the molecular features are the symmetrized atomic features, which are computed by
        ...
        \begin{equation}
			F_l := [s^l_1, s^l_2, ..., S^l_{11}, S^l_{12}, ... ] \in \mathbb{R}^{1 \times n_f^\text{mol}},
        \end{equation}
    	of size
    	\begin{equation}
    		n_f^\text{mol} := n_f(n_f-1)/2 + 2n_f.
    	\end{equation}
	...
	\end{equation}
	\item Store $F$, the storage should be bounded by $O(N)$ for most cases unless much more features are needed (although currently from experiment this causes overfitting).
\end{itemize}
